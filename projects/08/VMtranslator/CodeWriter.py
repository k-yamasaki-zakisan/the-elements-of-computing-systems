...
class CodeWriter():
    ...
    def write_push_from_virtual_segment(self, segment:str, index:str):
        if segment == "local":
            register_name = "LCL"
    ...
    def write_pop_from_virtual_segment(self, segment:str, index:str):
        if segment == "local":
            register_name = "LCL"
        elif segment == "argument":
            register_name = "ARG"
    ...
    def write_push_from_static_segment(self, segment:str, index:str):
        if segment == "temp":
    ...
    def write_pop_from_static_segment(self, segment:str, index:str):
        if segment == "temp":
            base_address = TEMP_BASE_ADDRESS
        elif segment == "pointer":
            base_address = POINTER_BASE_ADDRESS
    ...
    def write_call(self, function_name:str, num_of_args:str):
    ...
    def write_function(self, function_name:str, num_of_locals:str):
        self.write_codes([
            '(%s)' % function_name,
            'D=0'
        ])
        for _ in range(int(num_of_locals)):
            self.write_push_from_d_register()
